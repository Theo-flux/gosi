...
const PubCard = ({pub}) => {
  ...
  return(
    <div className="flex flex-col-reverse md:flex-row gap-x-4 justify-between items-start mb-16 last:mb-0">

      <div className="font-gilmer mt-8 md:w-[400px] md:mt-0 lg:w-[500px]">
        <h3 className="text-primary-900 text-3xl font-medium">{title}</h3>
        <p className="my-4">{data}</p>
        <p className="text-primary-900 text-xs font-bold">{date}</p>
      </div>

      <fig className="rounded-md overflow-hidden">
        <img className="w-[100%]" style={{objectFit: "cover"}}  src={image} alt="pub"/>
      </fig>
